...
layui.define(['layer', 'form'], function (exports) {
    ...
    var tools = {
        face: '<span title="插入表情"><i class="layui-icon" event="face" style="top:3px;">&#xe60c;</i><span event="face" class="tool-tip">表情</span></span>',
        image: '<span event="image" title="插入图片"><i class="layui-icon" event="image" style="top:3px;">&#xe60d;</i><span class="tool-tip">图片</span></span>',
        link: '<span event="link" title="插入链接"><i class="layui-icon" event="link" style="top:3px;">&#xe64c;</i><span class="tool-tip">链接</span></span>',
        code: '<span event="code" title="插入代码" class="divide"><i class="layui-icon" event="code" style="top:3px;">&#xe64e;</i><span class="tool-tip">代码</span></span>',
        full_screen: '<span event="full_screen" title="全屏"><i class="layui-icon" event="full_screen" style="top:2px;">&#xe632;</i><span class="tool-tip">全屏</span></span>',
        subfield: '<span event="subfield" title="分栏预览" class="hide"><i class="layui-icon" event="subfield" style="top:3px;">&#xe654;</i><span class="tool-tip">分栏</span></span>',
        preview: '<span event="preview" title="全屏预览"><i class="layui-icon" event="preview" style="top:2px;">&#xe630;</i><span class="tool-tip">预览</span></span>'
    };

    // 建立编辑器
    markdown.prototype.build = function (id_edit,id_preview,settings) {
        var dom = $('#' + id_edit);
        var md_preview = $('#'+ id_preview);
        ...
        var editor = $([
            '<div class="layui-markdown" id="layui-markdown-editor">',
            '  <div class="tools">' + _tools + '<span type="help" style="float: right; color:#FFFAFA"><small>Markdown Edit</small></span></div>',
            '  <textarea placeholder="请输入内容" class="layui-textarea layui-markdown-scroll" ' + _required + ' name="markdown_content" style="height: ' + _settings.height + ';resize:none;" id="layui-markdown-textarea"></textarea>',
            '</div>'].join('')
        );
        dom.empty().append(editor);
        //添加预览块
        var add_preview = $([
            '<div class="layui-markdown" id="layui-markdown-preview" style="height:100%;">',
            '<div style="height:40px;background:#64C8FF;border-radius:1px 0 0 0;"><p align="center" style="padding-top:5px;color:#FFFAFA;"><strong>实时预览</strong></p></div>',
            '<div class="layui-markdown-scroll" id="layui-markdown-show" style="height:95%;overflow-y:auto;border:1px solid #e6e6e6;background:#eeeeee;"></div>',
            '</div>'].join('')
        );
        md_preview.empty().append(add_preview);
        //监听textarea实现实时预览
        var parser = new HyperDown;
        $('#layui-markdown-textarea').bind('input propertychange', function() {
            var temp = $('#layui-markdown-textarea').val();
            var val  = parser.makeHtml(temp);
            $('#layui-markdown-show').html(val);
            Prism.highlightAll();
        });
        // 表情
        ...
        dom.find("span[event='full_screen']").click(function () {
            if (!is_full_screen) {
                ...
                dom.find("#layui-markdown-editor").addClass('full-screen');
                dom.find(".layui-markdown .tools span[event='subfield']").removeClass("hide");
            } else {
                $(".layui-body").css("position", 'absolute');
                dom.find("#layui-markdown-editor").removeClass('full-screen');
                dom.find(".layui-markdown .tools span[event='subfield']").addClass("hide");
                dom.find("#layui-markdown-editor").removeClass('half');
                $(".article-preview").removeClass("full");
                $("#layui-markdown-show").height($("#layui-markdown-textarea").height()-7);
        ...
        dom.find("span[event='subfield']").click(function () {
            ...
            if (!is_half_screen) {
                shade = $('<div class="layui-layer-shade" style="z-index:1110;"></div>')
                $("body").append(shade);
                dom.find(".layui-markdown").addClass('half');
                $("#layui-markdown-preview").addClass("half-right");
                $("#layui-markdown-show").height($("#layui-markdown-textarea").height()+12);
            } else {
                dom.find(".layui-markdown").removeClass('half');
                $("#layui-markdown-preview").removeClass("half-right");
                if (shade)
                    shade.remove();
            }
    ...
    var link = function (obj, text, callback) {
        ...
        var _height = ($(window).width() < 420) ? "260px" : "365px";
        return _layer.open({
            title: "插入链接：", type: 1, area: [_width, _height], btn: null, shadeClose: true, shade: 0.2,
            content: "<div style='margin-top: 20px; margin-left: 16px'>" +
    ...
    var face = function (obj, callback) {
        ...
        if ($(window).width() < 450) {
            ...
            $(".layui-util-face ul.layui-clear").css("width", "90%");
        }
        return _idx;
    };

    //markdown.prototype.getHtml = function () {
        //return $('#layui-markdown-textarea').val();
    //}
    //markdown.prototype.getMarkdown = function () {
        //return $('#layui-markdown-show').val();
    //}
